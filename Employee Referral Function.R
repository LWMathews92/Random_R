...
#   is measured by the size of an interaction term (int.var.c) which is the mean salary of a position by market and Experience divided by the mean salary of the entire
...
HR<-read.csv("data.csv") # 

####----************************** THE FOLLOWING VARIABLES MUST BE IN THE HR DATASET UNDER THE FOLLOWING NAMES *********************************************************************-------------#
# ID <- an Identification Variable, Job <- Position Identifier or Uniform Description, Market <- Market Identifier, Experience , Annual.Salary <- Annual Employee Salary,
...
HR.performance <- paste("X",strsplit((as.character(reference.date)),"-")[[1]][1],performance.var,sep = "")     #
#---------------------------------------------------------------------------------------------------------------#  
...
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------#
  HR.F <- HR %>%  mutate(Performance = extract_numeric(.[,match(HR.performance,names(.))])) %>% 
                  group_by(Market,Job,Experience) %>% 
                  mutate(mean.salary = mean(Annual.Salary), int.var.c = mean.salary / mean(HR$Annual.Salary)) %>%
                  ungroup()
